	...
	function registrar_ingreso()
	{
		...
		if($dato = $this->almacen_model->listaproductoselecionado($empresa,$id_usu))
		{
			foreach($dato as $val)
            {
             	...
             	$estado = "AC";
             	$tipoingreso = 0;
				$insert = $this->almacen_model->registrar_ingreso_almacen($id_usu,$id_prod,$idve_proveedor,$idve_porcentaje,$entrada,0,$entrada,$precio_compra,$precio_venta,$vencimiento,$fecha_vencimiento,$fecha,$fecha_hora,$factura,$num_factura,$observaciones,$estado,$tipoingreso);	
